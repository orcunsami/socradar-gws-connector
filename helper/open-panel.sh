...
if [ ! -f "$CFG" ]; then
  echo "deploy/customer.env not found. Run  bash helper/create-env.sh  first."; exit 1
fi
...
if [ -z "$PROJECT" ]; then
  echo "PROJECT is not set in deploy/customer.env. Run  bash helper/create-env.sh  first."; exit 1
fi
